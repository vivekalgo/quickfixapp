...
import 'package:quickfix/features/tracking/presentation/controllers/tracking_providers.dart';
...
class _LiveTrackingScreenState extends ConsumerState<LiveTrackingScreen> {
  int _currentStep =
      0; // 0: Confirmed, 1: Assigned, 2: Arriving/On the way, 3: Started, 4: Completed
  int _providerTime = 0;
  ...
  Future<void> _fetchBookingDetails() async {
    try {
      ...
      final data = await repository.getBookingDetails(widget.bookingId);

      final status = data['status']?.toString() ?? 'pending';
      final pName = data['providerName']?.toString() ?? 'Assigning Expert...';
      final custLat =
          double.tryParse(data['customerLat']?.toString() ?? '26.4912') ??
          26.4912;
      final custLng =
          double.tryParse(data['customerLng']?.toString() ?? '80.3156') ??
          80.3156;
      final provLat =
          double.tryParse(data['providerLat']?.toString() ?? '') ?? custLat;
      final provLng =
          double.tryParse(data['providerLng']?.toString() ?? '') ?? custLng;
      ...
      if (mounted) {
        ...
        if (_mapController != null) {
          _mapController!.animateCamera(CameraUpdate.newLatLng(dPos));
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Map<String, dynamic>> milestones = [
      ...
      {
        'title': 'Arriving / On the way',
        'subtitle': _bookingStatus == 'navigating'
            ? 'Expert is riding to your home'
      ...
      },
      {
        ...
        'subtitle': 'Rate & pay visiting charges',
        'icon': Icons.stars,
      },
    ];
                ...
                if (_bookingStatus != 'pending')
                  Marker(
                    ...
                    infoWindow: InfoWindow(
                      title: '$_providerName ($_providerTime mins away)',
                    ),
                    icon: BitmapDescriptor.defaultMarkerWithHue(
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                    'SOS Safety Alert Activated. Assistance is on the way.',
                  ),
  ...
  Future<void> _respondToQuotation(
    ...
    try {
      final repository = ref.read(trackingRepositoryProvider);
      final data = await repository.respondToQuotation(
      ...
      if (data['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              data['message'] ?? 'Failed to update quotation response.',
            ),
          ),
        );
      }
  ...
  Widget _buildQuotationReviewCard(bool isDark) {
    ...
    final double labour =
        (_quotation!['labourCharge'] as num?)?.toDouble() ?? 0.0;
    final double spares =
        (_quotation!['spareParts'] as num?)?.toDouble() ?? 0.0;
    final double materials =
        (_quotation!['additionalMaterials'] as num?)?.toDouble() ?? 0.0;
    final double visiting =
        (_quotation!['visitingCharges'] as num?)?.toDouble() ?? 0.0;
    final double discount =
        (_quotation!['discount'] as num?)?.toDouble() ?? 0.0;
    final double gst = (_quotation!['gst'] as num?)?.toDouble() ?? 0.0;
    final double total =
        (_quotation!['totalAmount'] as num?)?.toDouble() ?? 0.0;
