...
class _SplashScreenState extends ConsumerState<SplashScreen> {
  ...
  void didChangeDependencies() {
    ...
    // Watch auth state changes — will trigger when auth check finishes
    final authState = ref.read(authProvider);
    if (!authState.isLoading && _minDelayPassed) {
      _tryNavigate();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Reactively watch auth state — triggers rebuild when session check resolves
    final authState = ref.watch(authProvider);
    if (!authState.isLoading && _minDelayPassed && !_navigated) {
