...
final bookingReceiptProvider = FutureProvider.family<Map<String, dynamic>?, String>(
  (ref, bookingId) async {
    try {
      final client = ref.read(dioClientProvider);
      final response = await client.get(ApiEndpoints.bookingLedger(bookingId));
...
class BookingReceiptScreen extends ConsumerWidget {
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Booking ID copied!'), duration: Duration(seconds: 2)),
              );
            },
          ),
        ],
                ...
                Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      _receiptRow('Status', _statusLabel(payStatus), isDark, statusColor: _statusColor(payStatus)),
                      const Padding(
                      ...
                      // Pricing breakdown
                      _receiptRow('Service Amount', '₹${gross.toStringAsFixed(2)}', isDark),
                      if (gateway > 0)
                        _receiptRow('Gateway Charges', '+ ₹${gateway.toStringAsFixed(2)}', isDark,
        ...
        child: Column(
          children: [
            Container(
              ...
              child: Column(
                children: [
                  const Icon(Icons.receipt_long, color: Colors.white, size: 36),
                  const SizedBox(height: 8),
                  const Text('BOOKING RECEIPT', style: TextStyle(color: Colors.white70, fontSize: 11, letterSpacing: 1.5)),
                  const SizedBox(height: 4),
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text('Amount Paid', style: AppTextStyles.headingSmall(isDark)),
                      Text(
                        amount != null ? '₹${amount.toStringAsFixed(2)}' : 'See Booking Details',
            ...
            Container(
              ...
              child: const Center(
                child: Text('⚡ QuickFix — Powering Your Home Services', style: TextStyle(color: Colors.grey, fontSize: 11)),
              ),
            ),
          ],
        ),
          ...
          Flexible(
            child: Text(
              ...
              style: TextStyle(
                ...
                fontSize: 13,
                color: statusColor ?? valueColor ?? (isDark ? Colors.white : AppColors.secondary),
  ...
  Color _statusColor(String status) {
    switch (status) {
      ...
      case 'cash_collected':
      case 'settled':
      case 'settlement_pending': return AppColors.success;
      case 'cash_pending':
