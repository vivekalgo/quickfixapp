            ...
            SnackBar(
              content: Row(
                children: [
                  ...
                  Text(
                    'Coupon "$code" applied! You save ₹${discountAmt.toInt()}',
                  ),
                ],
              ),
...
class _OffersBottomSheetState extends ConsumerState<_OffersBottomSheet> {
  ...
  Future<void> _applyOffer(Map<String, dynamic> offer) async {
    ...
    try {
      ...
      final resData = await repository.calculateCheckout({
        'shopId': shopId,
        'items': itemsList,
        'couponCode': code,
      });
      if (resData['success'] == true) {
        final discountAmt =
            (resData['couponDiscount'] as num?)?.toDouble() ?? 0.0;
            ...
            child: widget.offers.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, index) {
                      ...
                      final baseColor =
                          _couponPalette[index % _couponPalette.length];
