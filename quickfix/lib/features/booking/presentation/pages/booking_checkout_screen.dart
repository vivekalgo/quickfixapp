...
import 'package:quickfix/features/booking/presentation/widgets/checkout_order_summary.dart';
...
class _BookingCheckoutScreenState extends ConsumerState<BookingCheckoutScreen> {
  ...
  void _handlePaymentError(PaymentFailureResponse response) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Payment failed: ${response.message ?? "User cancelled payment"} (Code: ${response.code})',
        ),
  ...
  void _executeBookingRequest(Map<String, dynamic> bookingData) async {
    ...
    try {
      ...
      if (resData['success'] == true) {
        ...
        final bookingId =
            resData['bookingId'] ??
            (resData['booking'] != null ? resData['booking']['id'] : null);
        final amount = resData['booking'] != null
            ? resData['booking']['amount']
            : null;
        final date = resData['booking'] != null
            ? resData['booking']['date']
            : null;
        final slot = resData['booking'] != null
            ? resData['booking']['slot']
            : null;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Failed to save booking order: ${ErrorHandler.handle(e, s).message}',
          ),
        ),
      );
    } finally {
              ...
              try {
                ...
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Failed to add address: ${ErrorHandler.handle(e, s).message}',
                      ),
                    ),
                  );
                }
  ...
  Widget build(BuildContext context) {
    ...
    final currentAddress = ref.watch(
      currentAddressProvider.select((state) => state.address),
    );

    final calcAsync = ref.watch(checkoutCalculationProvider);
    final calcData = calcAsync.value;

    final double finalAmount = calcData != null
        ? (calcData['grandTotal'] as num).toDouble()
        : 0.0;
    final double discount = calcData != null
        ? (calcData['couponDiscount'] as num).toDouble()
        : 0.0;

    final bool hasInspectionService = calcData != null
        ? (calcData['pricingType'] != 'fixed')
        : false;
    final bool isFreeInspection = calcData != null
        ? (calcData['isFreeInspection'] == true)
        : false;
    final double inspectionVisitingCharges = calcData != null
        ? (calcData['visitingCharge'] as num).toDouble()
        : 0.0;
    final double convenienceFee = calcData != null
        ? (calcData['convenienceFee'] as num).toDouble()
        : 0.0;
    ...
    final List<Map<String, String>> addresses = savedAddresses.map((addr) {
      ...
      if (details.toLowerCase().contains('office') ||
          details.toLowerCase().contains('work')) {
        type = 'Office';
      } else if (details.toLowerCase().contains('swaroop') ||
          details.toLowerCase().contains('home')) {
        type = 'Home';
      }
      return {'type': type, 'details': details};
    }).toList();

    final hasCurrentAddress = addresses.any(
      (element) => element['details'] == currentAddress,
    );
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            CheckoutOrderSummary(isDark: isDark, cart: cart),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text('To Pay', style: AppTextStyles.bodySmall(isDark)),
                ],
              ),
            ),
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _isProcessing
                    ? null
                    : () =>
                          _triggerPaymentGateway(context, finalAmount, isDark),
                style: ElevatedButton.styleFrom(
                  backgroundColor: AppColors.primary,
  ...
  void _triggerPaymentGateway(
    ...
    final hasCurrentAddress = addresses.any(
      (element) => element['details'] == currentAddress.address,
    );
    ...
    if (addresses.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Please add or select a delivery address before payment.',
          ),
          backgroundColor: AppColors.error,
        ),
      );
    ...
    final bool hasInspectionService = cart.values.any(
      (item) => item.pricingType == 'inspection',
    );
    ...
    final bookingData = {
      ...
      'pricingType': hasInspectionService
          ? 'inspection'
          : (cart.values.any((item) => item.pricingType == 'starting')
                ? 'starting'
                : (cart.values.any((item) => item.pricingType == 'range')
                      ? 'range'
                      : 'fixed')),
      'isFreeInspection': isFreeInspection,
      'items': cart.values
    ...
    if (methodParam == 'Razorpay' && amount > 0.0) {
      ...
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Failed to load Razorpay payment modal: ${resolved.message}',
            ),
          ),
        );
      }
