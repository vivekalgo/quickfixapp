...
class _EditProfileScreenState extends ConsumerState<EditProfileScreen> {
  ...
  static const List<String> _languages = ['English', 'Hindi', 'Tamil', 'Telugu', 'Bengali', 'Marathi'];
  ...
  void initState() {
    ...
    _altPhoneController = TextEditingController(text: user?['alternatePhone']?.toString() ?? '');
    _emergencyController = TextEditingController(text: user?['emergencyContact']?.toString() ?? '');
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      final XFile? file = await picker.pickImage(source: source, maxWidth: 512, maxHeight: 512, imageQuality: 75);
      if (file != null) {
        setState(() => _pickedImage = File(file.path));
      }
    } on PlatformException catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Permission denied: ${e.message}')));
      }
    }
  }
      ...
      builder: (_) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            if (ref.read(authProvider).user?['avatarUrl']?.toString().isNotEmpty == true || _pickedImage != null)
              ListTile(
                leading: Container(
                  padding: const EdgeInsets.all(8),
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      if (_pickedImage != null) {
        ...
        final mimeType = _pickedImage!.path.toLowerCase().endsWith('.png') ? 'image/png' : 'image/jpeg';
        ...
        actions: [
          if (!_isSaving)
            TextButton(
              ...
              child: const Text('Save', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold, fontSize: 15)),
            ),
        ],
      ),
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.phone_android_outlined, size: 18, color: AppColors.textSecondaryLight),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Text(
                        '+91 $phone',
                        style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: isDark ? Colors.white70 : AppColors.secondary),
                      ),
                    ),
                    Container(
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.cake_outlined, size: 18, color: AppColors.textSecondaryLight),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Text(
                          _dob != null
                      ...
                      Icon(Icons.calendar_month_outlined, size: 18, color: isDark ? AppColors.textSecondaryDark : AppColors.textSecondaryLight),
                    ],
                  ),
                ),
              ),
              ...
              OutlinedButton(
                ...
                child: const Text('Cancel', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold, fontSize: 16)),
              ),
  ...
  Widget _buildSectionHeader(String label, bool isDark, {bool optional = false}) {
    return Row(children: [
      Text(label, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: isDark ? Colors.white70 : AppColors.secondary)),
      if (optional) ...[
        const SizedBox(width: 6),
        Text('(Optional)', style: TextStyle(fontSize: 11, color: isDark ? AppColors.textSecondaryDark : AppColors.textSecondaryLight)),
      ],
    ]);
  }
  ...
  Widget _buildTextField({
    ...
    return TextFormField(
      ...
      maxLength: maxLength,
      style: TextStyle(fontSize: 15, color: isDark ? Colors.white : AppColors.secondary),
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<T>(
          ...
          hint: Row(children: [
            ...
            const SizedBox(width: 12),
            Text(hint, style: const TextStyle(color: AppColors.textSecondaryLight, fontSize: 14)),
          ]),
          isExpanded: true,
          ...
          items: items.map((item) => DropdownMenuItem<T>(
            ...
            child: Row(children: [
              Icon(icon, size: 18, color: AppColors.textSecondaryLight),
              const SizedBox(width: 12),
              Text(item.toString(), style: TextStyle(fontSize: 15, color: isDark ? Colors.white : AppColors.secondary)),
            ]),
          )).toList(),
          onChanged: onChanged,
        ),
