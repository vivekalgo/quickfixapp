...
class _AddressesScreenState extends ConsumerState<AddressesScreen> {
  ...
  Future<void> _setDefaultAddress(Map<String, dynamic> address) async {
    ...
    final currentList = List<dynamic>.from(user['savedAddresses'] ?? []).map((item) {
      ...
      builder: (context) => _AddressFormSheet(
        ...
        onSave: (addressData) async {
          ...
          if (existingAddress != null) {
            // Update existing
            final index = currentList.indexWhere((item) => item['id'] == existingAddress['id']);
            if (index != -1) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Saved Addresses', style: AppTextStyles.headingMedium(isDark)),
      ),
      body: Column(
        children: [
          Expanded(
...
class _AddressFormSheetState extends ConsumerState<_AddressFormSheet> {
  ...
  void initState() {
    ...
    _addressController = TextEditingController(text: addr?['address']?.toString() ?? '');
  ...
  Future<void> _fetchGPSLocation() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Could not get GPS location: $e'), backgroundColor: Colors.red),
        );
      }
    } finally {
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text(
                    widget.existingAddress != null ? 'Edit Address' : 'Add Delivery Address',
              ...
              OutlinedButton.icon(
                ...
                label: Text(_isLocating ? 'Locating...' : 'Use Current GPS Location'),
              ...
              Text(
                'Complete Address',
                style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: isDark ? Colors.white70 : AppColors.secondary),
              ),
              const SizedBox(height: 8),
              TextFormField(
              ...
              if (_selectedLabel == 'Other') ...[
                ...
                TextFormField(
                  ...
                  validator: (v) => _selectedLabel == 'Other' && (v == null || v.trim().isEmpty) ? 'Please enter a custom label' : null,
                  style: TextStyle(color: isDark ? Colors.white : AppColors.secondary, fontSize: 14),
                  decoration: InputDecoration(
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  final labelText = _selectedLabel == 'Other' ? _customLabelController.text.trim() : _selectedLabel;
                  final id = widget.existingAddress?['id']?.toString() ?? 'addr_${DateTime.now().millisecondsSinceEpoch}';
                  widget.onSave({
                    'id': id,
                    'label': labelText,
