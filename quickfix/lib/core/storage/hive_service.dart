...
/// Enterprise-grade Local Storage Service using the Hive binary database engine.
///
/// This service acts as the primary client-side cache and persistent configuration store.
/// It separates data into a public settings box (`app_settings`), a local notifications database (`local_notifications`),
/// and an encrypted cache box (`app_cache`) for secure token and profile storage.
class HiveService {
  ...
  /// 
  /// The key is stored in the device's secure hardware storage (Keychain on iOS, KeyStore on Android)
  /// using [FlutterSecureStorage]. If secure storage fails, it falls back to a derived static salt key.
  static Future<List<int>> _getOrCreateSecureKey() async {
    try {
      ...
      final keyStr = await secureStorage.read(key: 'hive_encryption_key');
      if (keyStr != null) {
    ...
    final part1 = base64.decode('UXVpY2tGaXhBcHBT');
    final part2 = base64.decode('ZWN1cmVMb2NhbFM=');
    final part3 = base64.decode('dG9yYWdlU2FsdDIwMjY=');
  ...
  /// 
  /// Sets up Hive paths, registers drivers, opens unencrypted settings and notification boxes,
  /// and opens the encrypted user caching box using a secure AES-256 cipher.
  static Future<void> init() async {
    await Hive.initFlutter();
    await Hive.openBox(_settingsBox);
    await Hive.openBox('local_notifications');
    final secureKey = await _getOrCreateSecureKey();
    await Hive.openBox(_cacheBox, encryptionCipher: HiveAesCipher(secureKey));
  }
  ...
  static bool isInitialPermissionFlowComplete() {
    final box = Hive.box(_settingsBox);
    return box.get('initial_permission_flow_complete', defaultValue: false)
        as bool;
  }
  ...
  static Future<void> saveActiveLocation(
    String address,
    double lat,
    double lng,
  ) async {
  ...
  static List<String> getFavouriteProviderIds() {
    final box = Hive.box(_cacheBox);
    final list =
        box.get('favourite_providers', defaultValue: <String>[]) as List;
    return list.map((e) => e.toString()).toList();
  }
  ...
  static List<String> getReadNotificationIds() {
    final box = Hive.box(_cacheBox);
    final list =
        box.get('read_notifications', defaultValue: <String>[]) as List;
    return list.map((e) => e.toString()).toList();
  }
  ...
  static List<String> getDeletedNotificationIds() {
    final box = Hive.box(_cacheBox);
    final list =
        box.get('deleted_notifications', defaultValue: <String>[]) as List;
    return list.map((e) => e.toString()).toList();
  }
  ...
  static Future<void> markMultipleNotificationsAsDeleted(
