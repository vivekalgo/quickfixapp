...
class _PaymentsScreenState extends ConsumerState<PaymentsScreen>
            ...
            children: [
              ...
              TextFormField(
                ...
                validator: (val) {
                  ...
                  if (numVal > maxAmount) return 'Exceeds available wallet balance';
                  return null;
                },
              ),
            ],
            ...
            onPressed: () async {
              ...
              if (success && context.mounted) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('✅ Settlement request submitted! Admin will review and transfer within 24–48 hrs.'),
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => ref.read(paymentsProvider.notifier).fetchEarnings(),
          ),
        ],
        bottom: TabBar(
          ...
          tabs: const [
            Tab(icon: Icon(Icons.account_balance_wallet_outlined, size: 18), text: 'Overview'),
            Tab(icon: Icon(Icons.swap_horiz, size: 18), text: 'Settlements'),
            Tab(icon: Icon(Icons.receipt_long_outlined, size: 18), text: 'Ledger'),
          ],
        ),
      ),
      body: RefreshIndicator(
        color: AppColors.primary,
        onRefresh: () async => ref.read(paymentsProvider.notifier).fetchEarnings(),
        child: paymentsState.errorMessage != null && !paymentsState.hasDashboardData
            ...
            : TabBarView(
                controller: _tabController,
                children: [
                  _buildOverviewTab(context, paymentsState, authState, isDark),
  ...
  Widget _buildOverviewTab(BuildContext context, PaymentsState state, dynamic authState, bool isDark) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(
                    ...
                    color: state.walletBalance < 0 ? Colors.redAccent : Colors.white,
                    fontFamily: 'Outfit',
                  ),
                ),
          ...
          // Commission Info Box
          _commissionInfoBox(state, authState, isDark),
          const SizedBox(height: 16),
          ...
          if (authState.shop != null) ...[
            ...
            const SizedBox(height: 10),
            _bankAccountCard(authState, isDark),
  ...
  Widget _earningsGrid(PaymentsState s, bool isDark) {
    return GridView.count(
      ...
      children: [
        _miniStatCard('Today\'s Earnings', s.todayEarnings, AppColors.success, Icons.trending_up, isDark),
        _miniStatCard('Cash Collected', s.todayCash, Colors.orange, Icons.money, isDark),
        _miniStatCard('Online Earnings', s.todayOnline, AppColors.primary, Icons.credit_card, isDark),
        _miniStatCard('Commission Deducted', s.todayCommission, Colors.redAccent, Icons.percent, isDark),
      ],
    );
  }
  ...
  Widget _overallStats(PaymentsState s, bool isDark) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          _statRow('Total Earnings', '₹${s.totalEarnings.toStringAsFixed(2)}', AppColors.success),
          _statRow('Total Commission Paid', '₹${s.totalCommission.toStringAsFixed(2)}', Colors.orange),
          if (s.commissionDue > 0)
            _statRow('Commission Outstanding', '₹${s.commissionDue.toStringAsFixed(2)}', Colors.red),
          _statRow('Cash Jobs', '${s.cashJobsCount} bookings', Colors.orange),
          _statRow('Online Jobs', '${s.onlineJobsCount} bookings', AppColors.primary),
          _statRow('Total Settled', '₹${s.totalSettled.toStringAsFixed(2)}', AppColors.success),
          _statRow('Completed Settlements', '${s.completedSettlementCount}', AppColors.success),
          if (s.pendingSettlementCount > 0)
            _statRow('Pending Settlement', '₹${s.pendingSettlementAmount.toStringAsFixed(2)} (${s.pendingSettlementCount})', Colors.orange),
        ],
      ),
    );
  }
  ...
  Widget _miniStatCard(String label, double value, Color color, IconData icon, bool isDark) {
    final bg = isDark ? AppColors.surfaceDark : AppColors.surfaceLight;
    final border = isDark ? AppColors.borderDark : AppColors.borderLight;
    return Container(
  ...
  Widget _commissionInfoBox(PaymentsState state, dynamic authState, bool isDark) {
    final bg = isDark ? AppColors.surfaceDark : AppColors.surfaceLight;
    final border = isDark ? AppColors.borderDark : AppColors.borderLight;
    return Container(
      padding: const EdgeInsets.all(16),
  ...
  Widget _bankAccountCard(dynamic authState, bool isDark) {
    final bg = isDark ? AppColors.surfaceDark : AppColors.surfaceLight;
    final border = isDark ? AppColors.borderDark : AppColors.borderLight;
  ...
  Widget _txCard(Map<String, dynamic> tx, bool isCredit, double amount, bool isDark) {
    final bg = isDark ? AppColors.surfaceDark : AppColors.surfaceLight;
    final border = isDark ? AppColors.borderDark : AppColors.borderLight;
    return Container(
      padding: const EdgeInsets.all(14),
      ...
      child: Row(
        children: [
          Container(
            ...
            height: 36,
            decoration: BoxDecoration(
              color: (isCredit ? AppColors.success : Colors.redAccent).withValues(alpha: 0.15),
              borderRadius: BorderRadius.circular(8),
            ),
            child: Icon(
  ...
  Widget _buildSettlementsTab(BuildContext context, PaymentsState state, bool isDark) {
    return SingleChildScrollView(
      physics: const AlwaysScrollableScrollPhysics(),
      padding: const EdgeInsets.all(16),
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                _settleSummaryItem('Total Settled', '₹${state.totalSettled.toStringAsFixed(0)}', AppColors.success),
                Container(width: 1, height: 40, color: Colors.white24),
                _settleSummaryItem('Pending', '₹${state.pendingSettlementAmount.toStringAsFixed(0)}', Colors.orange),
                Container(width: 1, height: 40, color: Colors.white24),
                _settleSummaryItem('Balance', '₹${state.walletBalance.toStringAsFixed(0)}',
          ...
          ElevatedButton.icon(
            ...
            label: const Text('Request New Settlement', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
          const SizedBox(height: 20),
          ...
          if (state.settlementHistory.isEmpty)
            FutureBuilder(
              future: ref.read(paymentsProvider.notifier).fetchSettlementHistory(),
              builder: (context, snap) {
                if (snap.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
                }
                if (ref.read(paymentsProvider).settlementHistory.isEmpty) {
                  return _emptyCard('No settlement history yet.\nRequest your first settlement above!', Icons.swap_horiz, isDark);
                }
                return _settlementList(ref.read(paymentsProvider).settlementHistory, isDark);
              },
            )
          else
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (s['transactionId'] != null && s['transactionId'].toString().isNotEmpty) ...[
                const SizedBox(height: 2),
                Text(
              ...
              if (s['adminNote'] != null && s['adminNote'].toString().isNotEmpty) ...[
  ...
  Widget _buildLedgerTab(BuildContext context, PaymentsState state, bool isDark) {
    return SingleChildScrollView(
      physics: const AlwaysScrollableScrollPhysics(),
      padding: const EdgeInsets.all(16),
      child: Column(
          ...
          if (state.ledgerEntries.isEmpty)
            FutureBuilder(
              ...
              builder: (context, snap) {
                ...
                if (entries.isEmpty) {
                  return _emptyCard('No ledger entries yet.\nComplete a booking to see accounting records.', Icons.receipt_long_outlined, isDark);
                }
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    _ledgerRow('Gross Billing Amount', '₹${gross.toStringAsFixed(2)}', Colors.white),
                    _ledgerRow('Platform Commission (${commRate.toStringAsFixed(0)}%)', '- ₹${commission.toStringAsFixed(2)}', Colors.orange),
                    const Divider(color: Colors.white12, height: 12),
                    _ledgerRow('Your Net Earnings', '₹${earnings.toStringAsFixed(2)}', AppColors.success, bold: true),
                  ],
                ),
              ),
              const SizedBox(height: 10),
              ...
              Row(
                children: [
                  _statusChip(_payStatusLabel(payStatus), _payStatusColor(payStatus)),
                  const SizedBox(width: 8),
                  _statusChip('Comm: ${commStatus.toUpperCase()}',
  ...
  Widget _ledgerRow(String label, String value, Color valueColor, {bool bold = false}) {
    return Padding(
  ...
  Widget _methodChip(String method) {
    ...
    return Container(
      ...
      child: Text(config[1] as String, style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: color)),
    );
  }
  ...
  Color _payStatusColor(String status) {
    switch (status) {
      case 'settled':
      case 'paid':
      case 'cash_collected': return AppColors.success;
